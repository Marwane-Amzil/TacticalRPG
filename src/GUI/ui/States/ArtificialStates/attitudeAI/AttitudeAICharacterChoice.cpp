...
AttitudeAICharacterChoice::AttitudeAICharacterChoice(StateMachine& machine, sf::RenderWindow& window, gui::World& world, utils::TextureManager& texture_manager, const bool replace)
...
void AttitudeAICharacterChoice::pause()
{
	std::cout << "pause choice\n";
}

void AttitudeAICharacterChoice::resume()
{
	std::cout << "resume choice\n";
}

void AttitudeAICharacterChoice::update()
{	
	sf::Event event;
	...
	if (count == 3)
	{
		_machine.run(StateMachine::build<AttitudeAIChoosePlay>(_machine, _window, _world, _textureManager, true));
	}
...
void AttitudeAICharacterChoice::draw()
{
    _window.clear();
    _window.draw(_world);
