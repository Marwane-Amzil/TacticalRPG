...
CharacterChoice::CharacterChoice(StateMachine& machine, sf::RenderWindow& window, gui::World& world, utils::TextureManager& texture_manager, iut::ClientSocket& client, const bool replace)
...
{	
	...
	if (count == 3)
	{
		_machine.run(StateMachine::build<ChoosePlay>(_machine, _window, _world, _textureManager, _client, true));
	}
